...
Generative AI - "The Gemini Sandwich":
  - Input (SIGNING_MODE): Google Gemini 3.0 Multimodal - interprets MediaPipe landmarks as ASL → English text.
  - Output (LISTENING_MODE): Google Gemini 3.0 - translates English text → ASL Gloss for avatar playback.
...
│   ├── avatar/
│   │   ├── AvatarPlayer.tsx   # Main avatar wrapper (mode: genasl | flipbook | legacy)
│   │   ├── GenASLPlayer.tsx   # AWS GenASL video-based avatar player
│   │   └── FlipbookPlayer.tsx # Canvas-based 24fps frame animation (fallback)
│   ├── ui/
...
│   ├── gemini/          # Gemini 3.0 API - "The Gemini Sandwich"
│   │   ├── prompts.ts       # ASL gloss translation prompts
│   │   ├── translationService.ts # English → ASL Gloss
│   │   ├── signRecognitionService.ts # Landmarks → English (Multimodal)
│   │   └── index.ts         # Barrel exports
│   ├── aws/             # AWS GenASL integration
│   │   ├── config.ts        # AWS environment configuration
│   │   ├── genASLService.ts # GenASL API client (Step Functions, S3, CloudFront)
│   │   └── index.ts         # Barrel exports
│   ├── elevenlabs/      # Audio conversion services
│   ├── avatar/          # Flipbook avatar system (legacy fallback)
...
Module B: The Gemini Sandwich (The Brain) 🥪
Responsibility: Gemini 3.0 powers BOTH sides of the bidirectional bridge.

**SIGNING_MODE Flow (Gemini as "The Eyes"):**
```
Camera → MediaPipe Landmarks → Gemini 3.0 Multimodal → English Text → ElevenLabs TTS → Audio
```
...
```
Gemini Gloss Output → GenASL API → Step Functions → Video Generation → CloudFront → Video Player
```
Logic:
1. Receive gloss array from Gemini translation.
2. Call GenASL API endpoint with text/gloss sequence.
3. GenASL orchestrates: Bedrock (optional re-translation) → Lambda → S3 → CloudFront.
...
```
Gloss Array → Supabase Storage → WebP Frames → Canvas (24fps)
```
...
Phase 3: Avatar Engine ✅ COMPLETE (Flipbook) / 🔄 IN PROGRESS (GenASL)
...
🔄 Integrate AWS GenASL for video-based avatar playback.
🔄 Build GenASLPlayer component for video rendering.
🔄 Implement genASLService for API communication.
...
Tasks:
Implement Gemini Multimodal for SIGNING_MODE (landmarks → English).
Implement Gemini translation for LISTENING_MODE (English → ASL gloss).
Connect GenASL to receive gloss from Gemini output.
Full loop: Sign → Gemini interprets → Audio output.
Full loop: Speak → Gemini translates → GenASL avatar plays.
...
graph TD
    ...
    subgraph Gemini_Sandwich [🥪 The Gemini Sandwich]
        GeminiEyes[Gemini 3.0 Multimodal<br/>ASL → English]
        GeminiLinguist[Gemini 3.0<br/>English → ASL Gloss]
    end
